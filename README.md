...
- True Web Push notifications through Supabase Edge Functions
- Background reminder delivery when the PWA/browser is not open
- Push delivery deduplication and stale-subscription cleanup
...
For background push notifications, also set `VAPID_PUBLIC_KEY` and follow `PUSH-NOTIFICATIONS-SETUP.md`.
...
The old foreground-only notification checker has been upgraded. This build includes a service-worker `push` handler, per-user Push subscriptions in Supabase, a scheduled Edge Function, and delivery deduplication.
